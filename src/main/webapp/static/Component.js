sap.ui.define([
		...
		return UIComponent.extend("sap.demo.bulletinboard.Component", {
			...
			/**
			 * Checks if the app runs in a local dev/test environment (e.g. local Tomcat).
			 * In that case, certain parts change the behavior a bit to simplify local
			 * development, e.g.
			 * - provide initial fake ads for easier testing, and
			 * - avoid sending data to the server because it doesn't have the "users"
			 *   service and thus cannot identify a "premium user" which is required to
			...
			_generateDummyAds : function() {
				var oAd1Purchased = new Date().getTime() - (365 * 24 * 60 * 60 * 1000);
				var oAd1Created = new Date().getTime() - (7 * 24 * 60 * 60 * 1000);
				var oAd1Updated = oAd1Created + (30 * 60 * 60 * 1000);
				var oAd2Purchased = new Date().getTime() - (500 * 24 * 60 * 60 * 1000);
				...
				var aAllAds = [
				    {
						...
						price : 9.95,
						currency : "EUR",
						purchaseDate : oAd1Purchased
				    },
				    {
						id : 2,
						createdAt : oAd2Created,
						updatedAt : oAd2Updated,
						version : 1,
